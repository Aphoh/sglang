...
def _exchange_fd(
    ...
    directory = tempfile.mkdtemp(prefix="sglang_vmm_fd_")
...
class RenewableVmmBuffer:
    ...
    def __init__(
        ...
        if device.type != "cuda" or device.index is None:
            raise ValueError("VMM buffer requires an indexed CUDA device")
        self.control_group: ProcessGroup | None = group
        self.device = device
    ...
    def restore(self) -> None:
        if self.state is VmmState.ATTACHED:
            return
        if self.state is not VmmState.DETACHED:
        ...
        try:
            ...
            received_fds = _exchange_fd(
                self.control_group, self.rank, self.world_size, local_fd
            )
            ...
            _check(
                ...
                "cuMemSetAccess(renewable buffer)",
                self.driver,
            )
            _check(
                self.driver.cuMemsetD8(self.local_ptr, 0, self.size),
                "cuMemsetD8(renewable buffer)",
                self.driver,
            )
        except Exception:
        ...
        self.state = VmmState.ATTACHED

    def close(self) -> None:
        if self.state is VmmState.CLOSED:
            return
        if self.state is VmmState.ATTACHED:
            self.detach()
        if self.state is not VmmState.DETACHED:
        ...
        _check(
            self.driver.cuMemAddressFree(self.base, self.span_size),
            "cuMemAddressFree(renewable buffer)",
            self.driver,
        )
        self.state = VmmState.CLOSED
